...
(function() {
	...
	[].forEach.call(speechInputWrappers, function(speechInputWrapper) {
		...
		var buttonSize = 0.7 * inputHeight;
		micBtn.style.top = 0.0 * inputHeight + 'px';
		...
		recognition.onresult = function(event) {
			...
			for (var i = event.resultIndex; i < event.results.length; ++i) {
				if (event.results[i].isFinal) {
					finalTranscript += event.results[i][0].transcript; lanciatore(finalTranscript); inputEl.value = finalTranscript = '';
				}
			}
